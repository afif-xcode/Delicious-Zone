...
import { getAllOrderAdmin } from '../../../../../services/operations/orderApi';
  ...
  return (
    <>
      ...
      <div className='flex flex-col gap-y-8'>
        ...
        {
          tab === "active" && (
            <OrderTable heading={"Active Orders"} data={userActiveOrders} loading={loading} setLoading={setLoading} />
          )
        }
        {/* Completed Orders */}
        {
          tab === "completed" && (
            <OrderTable heading={"Completed Orders"} data={userCompletedOrders} loading={loading} setLoading={setLoading}  />
          )
        }
        {/* Cancelled Orders */}
        {
          tab === "cancelled" && (
            <OrderTable heading={"Cancelled Orders"} data={userCancelledOrders} loading={loading} setLoading={setLoading}  />
          )
        }
      </div>
    </>
